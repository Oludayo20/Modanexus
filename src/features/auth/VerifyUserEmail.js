...
const VerifyUserEmail = () => {
  ...
  const canVerify = [userId, code].every(Boolean) || isLoading;
  ...
  let resendEmail = (
    <div className="text-red-500 text-2xl">
      <p>error Message:</p>
      <h1 className="pb-2">Error Verifying email</h1>
  ...
  return (
    <Helmet title="Verify-User-Email">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-5">
        {isLoading && <LoadingSpinner />}
        {isSuccess && (
          <ToastContainer
            messages={['Account verification successful!']}
            status={'success'}
          />
        )}
        {isError && (
        ...
        <div className="mb-4">
          <h3 className="text-2xl">Thanks for signing up for ModaNexus</h3>
